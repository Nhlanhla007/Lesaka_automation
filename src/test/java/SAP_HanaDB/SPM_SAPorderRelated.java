...
import Logger.Log;
//import evs_PageObjects.SPM_ProductSearch;
...
public class SPM_SAPorderRelated {
		WebDriver driver;
	    Action action;
	    DataTable2 dataTable2;
	    Base64Decoding decodePassword;
	     LinkedHashMap<String, LinkedHashMap<String, ArrayList<String>>> dataMap2 =null;
	    public SPM_SAPorderRelated(WebDriver driver,LinkedHashMap<String, LinkedHashMap<String, ArrayList<String>>> dataMap2,DataTable2 dataTable2) {
	        this.driver = driver;
		...
		public void SAP_OrderDetailVadidation(ExtentTest test) throws Exception{
			...
			String cartSum = dataTable2.getValueOnOtherModule("SPM_ProductSearch", "CartTotal", 0);  //"1595.0"; 
			String SAP_orderNo= dataTable2.getValueOnOtherModule("SPM_GenerateOrderSAPnumber","OrderSAPnumber",0); //"0005233897"; 
			String ExpPurchaseOrderNo =  dataTable2.getValueOnOtherModule("SPM_RetriveOrderID","orderID",0); // "66200000585";
			String ExpGrandTotal =String.valueOf(cartSum);//comes from cart total
			
//			List<String> ExpSku =new ArrayList<>();
//			String produts = "";
//			Map<String,List<String>> AllICprducts = spm_PageObjects.SPM_ProductSearch.productData;
...
//		  }
			...
			String ExpCITY= dataTable2.getValueOnOtherModule("SPM_DeliveryPopulation", "city", 0).toLowerCase().trim();
			//String ExpSTREET= ICDelivery.Streetname.toLowerCase().trim();//"33 baker street"; 
			String ExpSTREET = dataTable2.getValueOnOtherModule("SPM_DeliveryPopulation", "streetName", 0).toLowerCase().trim();
			//String ExpPostalcode = ICDelivery.Postalcode.trim();//"2196";
			String ExpPostalcode = dataTable2.getValueOnOtherModule("SPM_DeliveryPopulation", "postalCode", 0).trim();
			...
			if(allcheckpoint){
				...
				String isSkuPresent = dataTable2.getValueOnOtherModule("SPM_OrderStatusSearch", "IsBundleArticleSKUPresent", 0);
...
//					List<String> bundleArticleSKU =Arrays.asList(dataTable2.getValueOnOtherModule("SPM_OrderStatusSearch", "BundleArticleSKU", 0).split("#"));
