...
public class SAPorderRelated {
	...
	public void SAP_OrderDetailVadidation(LinkedHashMap<String, ArrayList<String>> input, ExtentTest test,
			int rowNumber) throws SQLException, IOException {
		...
		for (Map.Entry map : AllICprducts.entrySet()) {
			...
			// sum += (Integer.parseInt(quantity)*Integer.parseInt(price.replace("R",
			// "").replace(",", "")));
		...
		// "Select * from SAPEQ1.VBAK FULL OUTER JOIN SAPEQ1.VBAP ON
		// SAPEQ1.VBAK.VBELN=SAPEQ1.VBAP.VBELN WHERE SAPEQ1.VBAK.VBELN ='0005231326' ";

		String Query = "Select * from " + Schema + "." + Table1 + " FULL OUTER JOIN " + Schema + "." + Table2 + " ON "
				+ Schema + "." + Table1 + "." + key + " = " + Schema + "." + Table2 + "." + key + " WHERE " + Schema
				+ "." + Table1 + "." + key + " = '" + SAP_orderNo + "' ";
...
//          String Query= "SELECT * FROM SAPEQ1."+Table1+" WHERE "+key+" = '"+SAP_orderNo+"'";
		...
		if (rowsCountReturned >= 0) {
			...
			action.CompareResult(" SAP #Order :" + SAP_orderNo
					+ " SAP hana DB record count is greater than O, Populated rows:" + rowsCountReturned, "True",
					"True", test);
		...
		if (allcheckpoint) {
			...
			if (Float.parseFloat(ActualPrice) >= Float.parseFloat(ExpGrandTotal)) {
				...
				action.CompareResult(" Total Cart Price for all products in SAP DB " + "ActualPrice :" + ActualPrice
						+ " Expected :" + ExpGrandTotal, "True", "True", test);

			} else {

				action.CompareResult(" Total Cart Price for all products in SAP DB " + "ActualPrice :" + ActualPrice
						+ " Expected :" + ExpGrandTotal, "True", "False", test);
			...
			for (int k = 0; k < ExpProductName.size(); k++) {
				...
				action.CompareResult(" Products Purchased Description in SAP DB", eachProduct.trim().toUpperCase(),
						AllProductsNameDB.trim().toUpperCase(), test);
...
//               input.get("BP_Number").set(rowNumber,BPnumber);
		...
		// String Query= "Select * from "+Schema+"."+Table1+" FULL OUTER JOIN
		// "+Schema+"."+Table2+" ON "+Schema+"."+Table1+"."+key+" =
		// "+Schema+"."+Table2+"."+key+" WHERE "+Schema+"."+Table1+"."+key+" =
		// '"+SAP_orderNo+"' ";

		String Query1 = "select * from  SAPEQ1.ADRC where SAPEQ1.ADRC.ADDRNUMBER IN (select SAPEQ1.VBPA.ADRNR from SAPEQ1.VBPA where SAPEQ1.VBPA.VBELN = '"
				+ SAP_orderNo + "' and SAPEQ1.VBPA.PARVW = 'WE' and SAPEQ1.VBPA.POSNR<>'')";
