...
public class EVS_validateRegistrationForm {
	WebDriver driver;
    Action action;
    DataTable2 dataTable2;
    
    public EVS_validateRegistrationForm(WebDriver driver, DataTable2 dataTable2){
    	this.driver = driver;
    ...
    @FindBy(xpath = "//div[contains(text(),'Optional, required for TV licence validation. ID/P')]")
    ...
    public void validateRegForm(ExtentTest test) throws Exception{
    	...
    	if(action.elementExistWelcome(SAID, 2, "Check if the Identity textbox is available", test)){
    		...
    		String messageE = "Optional, required for TV licence validation. ID/Passport can be captured during a TV purchase too..";
